...
#if defined(CONFIG_LGE_FELICA) || defined(CONFIG_LGE_NFC_SONY_CXD2235AGG) || defined(CONFIG_LGE_IRDA)
...
#define MSM_GSBI4_PHYS		0x16300000
static struct msm_serial_hslite_platform_data apq8064_irda_gsbi4_pdata = {
	.config_gpio	= 1,
	.uart_tx_gpio	= 10,
	.uart_rx_gpio	= 11,
	.line		= 1,
...
*/
	...
	if(!lge_get_uart_mode()){
		pr_info("%s: uart off!, register irda platform \n",__func__);
		apq8064_device_uart_gsbi4.dev.platform_data = &apq8064_irda_gsbi4_pdata;
	}else{
		pr_info("%s: uart on!, no register irda platform \n", __func__);
	}
	#else
		apq8064_device_uart_gsbi4.dev.platform_data = &apq8064_irda_gsbi4_pdata;
...
*/
	...
	else /* Normal booting without uart enable */
	{
		apq8064_device_uart_gsbi4.id = 1;
		platform_add_devices(uart_devices, ARRAY_SIZE(uart_devices));
	}
	pr_info("[IRDA-DEBUG] apq8064_device_uart_gsbi4.id : %d",  apq8064_device_uart_gsbi4.id);
#else
	if (lge_get_uart_mode())
		platform_add_devices(uart_devices, ARRAY_SIZE(uart_devices));
#endif
